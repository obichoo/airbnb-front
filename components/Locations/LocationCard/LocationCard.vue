<template>
  <div class="w-[302px]" v-on:mouseenter="setIsMouseOver(true)" v-on:mouseleave="setIsMouseOver(false)">
    <div>
      <div class="relative rounded-xl h-[286px] overflow-hidden">
        <button v-on:click="setFavourite()" type="button" class="absolute z-20 top-3 right-4">
          <svg viewBox="0 0 32 32"
            :style="'display: block; fill: ' + isFavourite + '; height: 24px; width: 24px; stroke: white; stroke-width: 2; overflow: visible'">
            <path
        ...
        <div v-if="!isAtLimitLeft"
          class="absolute top-1/2 -translate-y-1/2 flex items-center justify-center h-20 w-20 left-0 z-10">
          <button id="swipe_left"
            ...
            <svg viewBox="0 0 32 32" class="block fill-none h-3 w-3 stroke-current overflow-visible pointer-events-none"
        ...
        <div v-if="!isAtLimitRight"
          class="absolute top-1/2 -translate-y-1/2 flex items-center justify-center h-20 w-20 right-0 z-10">
          <button id="swipe_right"
            class=" bg-white border-gray-300 border-2 rounded-full p-2 transition-all duration-300 shadow-none shadow-black hover:shadow-md"
            v-on:click="slideTo('right')">
            <svg viewBox="0 0 32 32" class="pointer-events-none block fill-none h-3 w-3 stroke-current overflow-visible"
              style="stroke-width: 5.33333">
              <g fill="none">
                <path
        ...
        <div class="absolute flex items-center justify-center bottom-3 z-20 left-1/2 -translate-x-1/2">
          <div v-for="(imgIndex, i) in location.imgs.length" :key="i" :class="{
...
export default {
  ...
  computed: {
    isFavourite() {
      return this.location.isFavourite ? 'rgba(255, 0, 0, 0.5)' : 'rgba(0, 0, 0, 0.5)'
    },
    imgIndexClasses() {
  ...
  methods: {
    ...
    setFavourite() {
      this.location.isFavourite ? (this.location.isFavourite = false) : (this.location.isFavourite = true)
    }
  }
}
</script>
